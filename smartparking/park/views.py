...
def details(request):
     if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('thankyou')  # Redirect to a thank you page after saving
     else:
        form = VehicleLogForm()
     return render(request, 'details.html',{'form':form})
def vehicle_log_list_view(request):
    logs = VehicleLog.objects.all()
    return render(request, 'vehicle_log_list.html', {'logs': logs})
